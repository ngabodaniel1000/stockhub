  ...
  const handleLogin = async (event) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8889/api/account/adminlogin",
        {
        ...
        <div className="w-full max-w-md">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input
                  ...
                  name="remember-me"
                  type="checkbox"
                  className={`h-4 w-4 rounded ${darkmode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-300'}`}
                />
          ...
          <div className="mt-6 text-center">
            <p className={`text-sm ${darkmode ? 'text-gray-400' : 'text-gray-600'}`}>
              ...
              <a href="/adminregister" className={`font-medium ${darkmode ? 'text-blue-400' : 'text-blue-600'}`}>
                Register
              </a>
            </p>
            <p className={`mt-1 text-sm ${darkmode ? 'text-gray-400' : 'text-gray-600'}`}>
              Manager?{' '}
              <a href="/managerlogin" className={`font-medium ${darkmode ? 'text-blue-400' : 'text-blue-600'}`}>
                Login as Manager
              </a>
            </p>
          </div>
